...
import { postDebtSettlement } from '../api/debtApi';
...
export function CustomerDebtSettlementModal({ open, onClose, contactId, contactName, currentDebt, onSettled }: Props) {
  ...
  const mut = useMutation({
    mutationFn: () => postDebtSettlement(contactId, { amount: to2dp(amount.trim()), notes: notes.trim() || undefined }),
    onSuccess: (res) => {
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-navy-950/80 p-4 backdrop-blur-sm" role="dialog" aria-modal="true" aria-label="سداد دين العميل">
      <div className="w-full max-w-md overflow-hidden rounded-2xl border border-navy-border/40 bg-navy-900">
        <div className="flex items-center justify-between border-b border-navy-border/30 p-4">
          <h3 className="text-sm font-bold text-slate-100">سداد دين — {contactName}</h3>
        ...
        <div className="space-y-3 p-4">
          <div className="rounded-xl border border-amber-500/25 bg-amber-500/[0.07] px-3 py-2">
            <p className="text-xs text-slate-400">الدين الحالي</p>
            <p dir="ltr" className="font-mono text-lg font-bold text-amber-400">{to2dp(currentDebt)} د.ج</p>
          </div>

          <div className="flex flex-wrap gap-1">
            {QUICK_AMOUNTS.map((q) => (
              <button key={q} type="button" onClick={() => setAmount(q)} className="h-7 rounded-lg border border-navy-border/40 bg-white/[0.03] px-2 py-0.5 font-mono text-xs text-slate-300 hover:border-cyan-500/40">{q}</button>
            ))}
            <button type="button" onClick={() => setAmount(to2dp(currentDebt))} className="h-7 rounded-lg border border-emerald-500/30 bg-emerald-500/10 px-2 py-0.5 text-xs font-bold text-emerald-400">سداد كامل</button>
          ...
          <div>
            ...
            <input type="text" value={notes} onChange={(e) => setNotes(e.target.value)} placeholder="ملاحظة…" className="mt-1 w-full rounded-xl border border-navy-border/40 bg-navy-950/60 px-3 py-2 text-sm text-slate-100 outline-none focus:border-cyan-500/50" />
          </div>
          ...
          {successData ? (
            <div id="customer-debt-receipt" className="rounded-xl border border-emerald-500/30 bg-emerald-500/10 p-3 text-center">
              <p className="text-sm font-bold text-emerald-300">تم السداد بنجاح</p>
              <p dir="ltr" className="font-mono text-xs text-slate-400">المسدد: {successData.amount} د.ج — المتبقي: {successData.remaining} د.ج</p>
            ...
            <button type="button" disabled={!amountValid || mut.isPending} onClick={() => mut.mutate()} className="w-full rounded-xl bg-emerald-600 px-4 py-2.5 text-sm font-extrabold text-white hover:bg-emerald-500 disabled:opacity-40">
              {mut.isPending ? 'جاري السداد…' : 'تأكيد السداد'}
            </button>
          )}
        </div>
      </div>
