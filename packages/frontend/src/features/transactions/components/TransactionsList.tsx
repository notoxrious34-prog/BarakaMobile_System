...
function getContactName(tx: Transaction, map: Record<string, string>): string {
  // Prefer denormalized contact, else account.contact, else map via contactId/account.contactId
  ...
  // fallback: try accountId -> contactId via map if we stored contactId there
  if (tx.account?.contactId && map[tx.account.contactId]) return map[tx.account.contactId];
  return tx.account?.contactId ?? tx.contactId ?? '—';
}

export function TransactionsList({ transactions, contactNameMap, onView }: Props) {
